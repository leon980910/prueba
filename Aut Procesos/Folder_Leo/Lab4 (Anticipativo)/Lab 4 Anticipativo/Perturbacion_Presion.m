...
F25=0.25*(max(P)-min(P))+5.27;
F50=0.50*(max(P)-min(P))+5.27;
F75=0.75*(max(P)-min(P))+5.27;
FF=[F25 F50 F75];
...
Gff=-((2.51*s-0.0003009)/(0.0003909-8.624*s))*exp(-7.06*s)
tmF=17.17
[Nff,Dff]=pade(tmF,5);
Grff = tf(Nff,Dff);
...
ts=5% se le pone un tiempo de establecimiento deseado
Mp=5% un overshoot de 5% 
sigma=4/ts;  %haciendo uso de la ecuacion general de diseño
% de segundo orden, se calculan los factores there, signam, frecuencia
% entre otros.
wd=(-pi*sigma)/log(Mp);
[np,dp]=tfdata(TF,'v');%se divide la ecuacion general con un numerador y un denominador
[zP,pP,kP]=tf2zp(np,dp);% Se Saca de la ecuacion los polos y ceros para calcular los factores
% siguienetes que ya tienen que ver con el diseño dle controlador PI
rs= (-180+(atand(abs(wd)/(sigma-abs(pP)))));
a=(abs(wd)/tand(-180-rs))+sigma;
Ki=sqrt((abs(wd)^2)+(sigma-abs(pP))^2)/(kP*sqrt((abs(wd)^2)+(a-sigma)^2));
PI=Ki*((s+a)/s)