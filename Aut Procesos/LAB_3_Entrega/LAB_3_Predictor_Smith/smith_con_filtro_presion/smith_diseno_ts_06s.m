%Diseño PI de presion con un ts de 0.6s
...
syms kd ;
eqn_1 = kd*(5.0096e-05)*(sqrt(wd^2+(a-sigma)^2))/((sqrt(wd^2+sigma^2))*(sqrt(wd^2+(p1-sigma)^2))) ==1 ;
...
figure
plot(salida_c)
title(['Salida de la Bomba en ecuaciones de diferencia'])
legend('Bomba','Location','SouthEast')
xlabel('iteraciones') 
ylabel('Consigna Bomba')
